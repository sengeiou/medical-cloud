...
public class TokenEncryptUtils {
	...
	public static String encryptData2MD5(String data) throws Exception {
		...
		try {
			...
			encryptResult = data;
			for (int i = 0; i < n; i++) {
				encryptResult = SecurityUtils.encryptMD5(encryptResult);
			}
		} catch (Exception e) {
			encryptResult = null;
			e.printStackTrace();
	...
	public static String sxyEncryMD5(String data) {
		...
		String resData = SecurityUtils.encryptMD5(data, times);
		return resData.toUpperCase();
	}
	/**
	 * Token加密
	 * 
	 ...
	 * @throws Exception
	 */
	public static String encryptToken(String phoneData, String pipelinekey) throws Exception {
		char[] pchars = phoneData.toCharArray();
		int phonel = pchars.length;
		int phonesum = 0;
		for (int i = 0; i < phonel; i++) {
		...
		return SecurityUtils.encryptDES(phoneData, enpipelinekey);
	}
	
	/**
	 * 将随机生成的4位数插入到MD5加密后的32位串的4个不同位置
	 * 
	 * @param encryptResult MD5加密后的32位串
	 * @param randomNumber 随机生成的4位数
	 * @param params 4个不同位置
	 * @return 返回36位字符串
	 * @throws Exception
	 */
	public static String insertRandomNumber(String encryptResult, String randomNumber, int[] params) throws Exception {
		// long start_time = System.currentTimeMillis();
		String result = null;
		try {
			char[] char_encrypt_result = encryptResult.toCharArray();
			char[] char_random_number = randomNumber.toCharArray();
