...
//		param.put("age", age);
	...
	/**
	 * 根据memberId和patientId查询会员患者信息
	 * 
	 * @param phone
	 * @return JSONObject
	 */
	public static JSONObject findMemberPatient(String memberId, String patientId) {
		JSONObject memberPatient = new JSONObject();
		//查询九州通会员信息
		JSONObject memberInfo = null;
		boolean isExist_member = false;
		String jsonStr = MemberUserUtils.findMember("", memberId,"");
		if (jsonStr != null) {
			JSONObject json = JSONObject.parseObject(jsonStr);
			String status = json.getString("status");
			if ("0".equals(status)) {
		...
		if (jsonStr2 != null) {
			...
			if ("0".equals(status)) {
				...
				if (jsonArray != null && jsonArray.size() > 0) {
					for (int i=0; i<jsonArray.size(); i++) {
						JSONObject patientUser_ = (JSONObject) jsonArray.get(i);
						if (patientUser_.getString("patientId").equals(patientId)) {
							patientUser = patientUser_;
							isExist_patient = true;
							break;
						}
		...
		String phone = patientUser.getString("phone");
		String patientName = patientUser.getString("name");
		String gender = patientUser.getString("gender");
		String age = patientUser.getString("age");
		String occupation = patientUser.getString("occupation");
