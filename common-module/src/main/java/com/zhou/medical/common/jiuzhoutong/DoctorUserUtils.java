...
//	private final static String findDoctorUrl = Constant.URL+"/out/findDoctor?";
	...
	* @throws
	...
	public static JSONObject findDoctorInfo(String phone) {
		...
		JSONArray feesBean = new JSONArray();
		...
		if (jsonStr != null) {
			...
			if ("0".equals(status)) {
				...
				if (jsonArray != null && jsonArray.size() > 0) {
					doctorInfo = (JSONObject) jsonArray.get(0);//医生实体
					//查询该医生费用信息
					if (feesArray != null && feesArray.size() > 0) {
						for(int i=0; i<feesArray.size(); i++){
							JSONObject fee_obj = (JSONObject) feesArray.get(i);
							if (doctorInfo.getString("doctorId").equals(fee_obj.getString("doctorId"))) {
								feesBean.add(fee_obj);
							}
						}
					}
		...
		doctorEntity.put("feesBean", feesBean);
				...
				if (jsonArray != null && jsonArray.size() > 0) {
					for (int i=0; i<jsonArray.size(); i++) {
						...
						if (doctorInfo_.getString("doctorId").equals(doctorId)) {
							//查询该医生费用信息
							if (feesArray != null && feesArray.size() > 0) {
								for(int j=0; j<feesArray.size(); j++){
									JSONObject fee_obj = (JSONObject) feesArray.get(j);
									if (fee_obj.getString("doctorId").equals(doctorId)) {
										feesBean.add(fee_obj);
									}
								}
							}
	...
	/**
	 * 根据doctorId查询返回医生JSONObject实体
	 * @param doctorId
	 * @param doctorName
	 * @return JSONObject
	 */
	public static JSONObject findDoctorInfoByDoctorId(String doctorId) {
		JSONObject doctorEntity = new JSONObject();
		//医生信息查询
		JSONObject doctorInfo = null;
		JSONArray feesBean = new JSONArray();
		boolean isExist_doctor = false;
		String jsonStr = DoctorUserUtils.findDoctor(doctorId,"", "", "","");
		if (jsonStr != null) {
			JSONObject json = JSONObject.parseObject(jsonStr);
			String status = json.getString("status");
			if ("0".equals(status)) {
	 ...
	 * @return JSONObject
	 */
	public static JSONObject findDoctorInfoByDoctorId2(String doctorId) {
		JSONObject doctorEntity = new JSONObject();
		//医生信息查询
		JSONObject doctorInfo = null;
		boolean isExist_doctor = false;
