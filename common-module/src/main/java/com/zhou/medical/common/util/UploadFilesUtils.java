...
public class UploadFilesUtils {
     ...
     * @return
     */
	public static String saveFiles(UserInfo userInfo, HttpServletRequest request,
								   MultipartFile[] files) {
		String urls = "";
		...
		if (files != null && files.length > 0) {
			...
			for (int i = 0; i < files.length; i++) {
				// 保存文件
				url = saveFile(userInfo, request, files[i]);
	 ...
	 * @param file
	 */
	public static String saveFile(UserInfo userInfo, HttpServletRequest request,
                                  MultipartFile file) {
		...
		try {
			if (file != null) {
				...
				url = formUploadFileToServer(userInfo, request, targetFile).trim();
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static String saveFiles(UserInfo userInfo, HttpServletRequest request,
		...
		if (files != null && files.length > 0) {
			...
			for (int i = 0; i < files.length; i++) {
				// 保存文件
				url = saveFile(userInfo, request, files[i], prefix);
				if (StringUtils.isNotBlank(url)) {
					urls += SEPARATOR + url;
				}
			}
     ...
     * @return
     */
	public static String formUploadFileToServer(UserInfo userInfo, HttpServletRequest request, File file) {
