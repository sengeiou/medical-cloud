...
public class ShiroDbRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(
        ...
        SystemUser user = systemUserFeignClient.findByParamFromSelectByUserName(username);
        if (user == null) {
            // 用户名不存在抛出异常
...
//        SystemUser user = new SystemUser();
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        System.out.println("roleListroleListroleListroleListroleList:"+roleList);
        Set<String> urlSet = new HashSet<String>();
        for (Integer roleId : roleList) {
        	//根据角色id查找资源url列表
        	List<SystemResource> resourceList = systemResourceFeignClient.findResourceUrlListByRoleId(roleId);
